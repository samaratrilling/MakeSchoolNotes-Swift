...
> [action]
> Select `Dashboard Scene` then select the `Navigation Item` inside the scene. Ensure you have the *Attributes Inspector* open and rename *Title* to *Dashboard*.
>
...
> [action]
> Select the `Add Bar Button Item` (the plus button you just added) then *Ctrl-Drag* this to the `New Note View Controller`
>
...
> Ensure the *Attributes Inspector* is selected. Set the identifier to 'Cancel'.
...
input let's create our process to save notes.
...
You are using a switch statement. Although for only one case you would typically use an `if` statement, we will be expanding this `switch` statement with additional use cases.
